...
class RequestUpdate: ObservableObject {
    ...
    func addRequest(request: String, name: String) {
        ...
        db.collection("RequestUpdate").addDocument(data: ["request":request, "name":name]) { error in
    ...
    func getData() {
        ...
        db.collection("RequestUpdate").getDocuments { snapshot, error in
